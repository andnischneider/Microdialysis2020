...
```
The sanity check looks good, all primer sequences have been removed from the data (weirdly it still finds one, but I don't think that's gonna be an issue).
...
```{r}
seqtab.sw <- as.data.frame(cbind(Cluster=clus_swarm$Cluster[match(gsub(";size=\\d+", "", names(seqs_clean_sum)), clus_swarm$ASV)], t(seqtab.nc5)))

seqtab.sw <- group_by(seqtab.sw, Cluster) %>% 
  summarise_each(funs(sum)) 
seqtab.sw$Cluster <- paste0("Cluster", seqtab.sw$Cluster)
seqtab.sw2 <- seqtab.sw
rownames(seqtab.sw2) <- seqtab.sw$Cluster
seqtab.sw2 <- data.matrix(seqtab.sw2[,-1])
seqtab.sw3 <- seqtab.sw2
```
...
names(seqs_clean_swarm) <- seqtab.sw$Cluster 

rownames(seqtab.sw3) <- as.character(seqs_clean_swarm)
seqtab.sw3 <- t(seqtab.sw3)

#Link UNITE database
unite_db <- ("/mnt/picea/storage/reference/Qiime/unite/UNITE_8.2/sh_general_release_dynamic_s_04.02.2020.fasta")
taxa <- assignTaxonomy(seqtab.sw3, unite_db, tryRC = TRUE, multithread = TRUE)
```
...
#Clean taxa
taxa <- gsub("^[[:alpha:]]__", "", taxa)
#Create clean matrix
rownames(seqtab.sw2) <- paste0("SOTU", 1:nrow(seqtab.sw2))

taxa <- as.data.frame(cbind(SOTU=rownames(seqtab.sw2), sequence=rownames(taxa), taxa))

seqtab.sw4 <- seqtab.sw2

#Adjust tax table
taxa <- taxa[taxa$SOTU%in%rownames(seqtab.sw4),]
rownames(taxa) <- taxa$SOTU
